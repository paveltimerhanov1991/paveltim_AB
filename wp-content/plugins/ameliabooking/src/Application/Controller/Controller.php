...
abstract class Controller
{
    ...
    public function __construct(Container $container)
    {
        ...
        DateTimeService::setTimeZone(
            $container->getSettingsService()
        );
    }
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        if ($commandResult->getUrl() !== null) {
            try {
                ...
                $responseBody['data']['emailError'] = true;
            }
        ...
        if ($commandResult->isIcs() === true) {
            /** @var Response $response */
            $response = $response->withHeader('Content-Type', 'text/calendar; charset=utf-8');
            $response = $response->withHeader('Content-Disposition', 'inline; filename="cal.ics"');
        }
