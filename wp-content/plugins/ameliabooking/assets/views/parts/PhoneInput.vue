...
<script>
  export default {
    ...
    mounted () {
      if (this.value !== '') {
        if (this.savedPhone) { this.fillInputWithSavedPhone() } else { this.formatPhoneNumber() }
      } else if (this.savedPhone) { this.fillInputWithSavedPhone() }
    },
      ...
      formatPhoneNumber () {
        ...
        if (this.input !== '') {
          if (this.input.startsWith('+')) {
            let country = this.countries.find(x => x.phonecode === parseInt(this.input.slice(1)))
