    ...
    <div class="am-dialog-scrollable" v-if="!dialogLoading">
      ...
      <el-form :model="customer" ref="customer" :rules="rules" label-position="top" @submit.prevent="onSubmit">
        ...
        <el-form-item :label="$root.labels.first_name+ ':'" prop="firstName">
          <el-input v-model="customer.firstName" auto-complete="off" @input="clearValidation()"></el-input>
        </el-form-item>

        <!-- Last Name -->
        <el-form-item :label="$root.labels.last_name + ':'" prop="lastName">
          <el-input v-model="customer.lastName" auto-complete="off" @input="clearValidation()"></el-input>
        </el-form-item>
        ...
        <el-form-item :label="$root.labels.date_of_birth + ':'">
          <v-date-picker
              ...
              :available-dates="{ end: (this.$moment().toDate()).setDate((this.$moment().toDate()).getDate() - 1) }"
    ...
    created () {
      if (this.customer.id !== 0) {
        ...
        if (this.customer.gender) {
          this.customer.gender = this.capitalizeFirstLetter(this.customer.gender)
        }
      } else {
        this.getWPUsers(0)
      }
    ...
    methods: {
      ...
      getParsedEntity () {
        ...
        if (customer.birthday) {
          customer.birthday = this.getDatabaseFormattedDate(customer.birthday)
        }
      ...
      getWPUsers: !AMELIA_LITE_VERSION ? function (currentId) {
