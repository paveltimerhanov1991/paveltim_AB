                        ...
                        <el-col :lg="9">
                          <el-row :gutter="10" class="am-appointments-flex-row-middle-align">
                            ...
                            <el-col :lg="7" :sm="6" :xs="16" v-if="$root.settings.role !== 'customer'">
                              <div class="am-appointment-status" @click.stop>
                                ...
                                <el-select
                                    ...
                                    :disabled="app.past || updateStatusDisabled || !canWriteAppointments()"
                                >
                                  <el-option
                            ...
                            <el-col :lg="12" :sm="6" v-if="$root.settings.role === 'customer'">
                              <span
                                  :class="'am-customer-status ' + app.bookings[0].status">{{ app.bookings[0].status }}</span>
      ...
      <transition name="slide">
        ...
        >
          <dialog-appointment
              ...
              @saveCallback="getAppointmentOptions"
...
<script>
  ...
  export default {
    mixins: [liteMixin, paymentMixin, entitiesMixin, appointmentMixin, imageMixin, dateMixin, durationMixin, notifyMixin, customerMixin, priceMixin, helperMixin, appointmentPriceMixin],
    ...
    methods: {
      ...
      deleteAppointmentGroup () {
        ...
        Object.keys(this.appointmentsDay).forEach(function (dateKey) {
          $this.appointmentsDay[dateKey].appointments.forEach(function (app) {
            if (app.checked) {
              $this.form.delete(`${$this.$root.getAjaxUrl}/appointments/` + app.id)
                .then(() => {
      ...
      canWriteAppointments () {
        return this.$root.settings.capabilities.canWriteOthers === true || (this.$root.settings.role === 'provider' && this.$root.settings.general.allowWriteAppointments)
      }
    },
