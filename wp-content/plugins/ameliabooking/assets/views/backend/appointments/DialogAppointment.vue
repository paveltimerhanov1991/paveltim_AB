<template>
  <div>
    ...
    <!-- Dialog Content -->
    <div class="am-dialog-scrollable" v-if="appointment && !dialogLoading">
      ...
      <el-form v-if="mounted" :model="appointment" ref="appointment" :rules="rules" label-position="top">
        <el-tabs v-model="newAppointmentTabs">
          ...
          <el-tab-pane :label="$root.labels.schedule" name="schedule">
            ...
            <!-- Customer -->
            <el-form-item :label="$root.labels.customers_singular_plural + ':'" prop="bookings">
              <el-select
            ...
            <transition name="fade">
              <div v-if="appointment.bookings.length > 0" class="am-selected-dropdown-items">
                ...
                <div v-for="(booking, index) in appointment.bookings" :key="index"
                  ...
                  <el-row align="middle" :gutter="4" justify="left">
                    ...
                    <el-col :span="14">
                      <h3>{{ (user = getCustomerInfo(booking)) !== null ? user.firstName + ' ' + user.lastName : '' }}</h3>
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12" class="v-calendar-column">
                <el-form-item :label="$root.labels.date + ':'" prop="selectedDate">
                  <v-date-picker
                      @input="dateChange"
              ...
              <!-- Time -->
              <el-col :span="12">
                <el-form-item :label="$root.labels.time + ':'" prop="selectedPeriod.time">
                  <el-select
              ...
              <div
                ...
                <div class="total">
                  ...
                  <el-row class="am-strong" :gutter="10">
                    ...
                    <el-col :span="10" class="align-right ">{{ getAppointmentPrice(appointment.bookings) }}
                    </el-col>
                  </el-row>
                </div>
              </div>
          ...
          <!-- Payment -->
          <el-tab-pane :label="$root.labels.payment" name="payment" v-if="appointment.id !== 0">
            <dialog-appointment-payment :appointment="appointment"
                                        @editPayment="editPayment"></dialog-appointment-payment>
          </el-tab-pane>

          <!-- Custom Fields -->
          <el-tab-pane :label="$root.labels.custom_fields" name="customFields" v-if="showCustomFieldsTab()">
    ...
    <!-- Dialog Actions -->
    <dialog-actions
        v-if="appointment && !dialogLoading"
...
<script>
  import Filter from '../../../js/backend/classes/Filter'
  ...
  export default {
    ...
    methods: {
      ...
      instantiateDialog () {
        if ((this.appointment !== null || (this.appointment !== null && this.appointment.id === 0)) && this.executeUpdate === true) {
          this.filter = new Filter(this.entitiesChoices, this.options.entities)
          ...
          if (this.appointment.id !== 0) {
            ...
            this.filter = new Filter(this.entitiesChoices, this.options.entities)
            this.filter.process('employees', this.appointment.providerId, ['locations', 'categories', 'services'])
            this.filter.process('locations', this.appointment.locationId, ['categories', 'employees', 'services'])
            this.filter.process('services', this.appointment.serviceId, ['locations', 'employees', 'categories'])
            this.filter.process('categories', this.appointment.categoryId, ['locations', 'employees', 'services'])
            ...
            this.getTimeSlots(function (timeSlots) {
              ...
              if (!(selectedDate in timeSlots)) {
                ...
                timeSlots[selectedDate][selectedTime] = [this.appointment.providerId]
      ...
      setLocation () {
        this.appointment.locationId = this.options.entities.employees.filter(employee => this.appointment.providerId === employee.id)[0].locationId
      },
      ...
      unSelectEntities () {
        for (let key in this.options.entities) {
          if (this.options.entities.hasOwnProperty(key)) {
            this.options.entities[key].forEach(function (item) {
              item.disabled = false
            })
          }
        }
              ...
              if (['approved', 'pending'].includes(booking.status)) {
                ...
                booking.extras.forEach(function (extItem) {
                  if (extItem.selected) {
                    ...
                    bookingExtrasTotalPrice += booking.persons * (extItem.quantity ? extItem.quantity : 0) * extraPrice
                  }
                })
                ...
                booking.serviceTotalPrice = servicePricePrice * booking.persons
    ...
    computed: {
      locationsFiltered () {
        return this.options.entities.locations.filter(
          location => location.status === 'visible' ||
            (this.appointment.id && this.appointment.providerId && location.status === 'hidden' && this.getProviderById(this.appointment.providerId).locationId === location.id)
        )
      },
      ...
      employeesFiltered () {
        ...
        employees = employees.filter(employee => employee.serviceList.filter(service => service.status === 'visible').length > 0 || employee.id === this.appointment.providerId)

        // Show only employees which location has status "visible"
        if (this.locationsFiltered.length > 0) {
          employees = employees.filter(employee => (employee.locationId !== null ? this.getLocationById(employee.locationId).status === 'visible' : false) || employee.id === this.appointment.providerId)
        }

        return employees.filter(employee => employee.status === 'visible' || employee.id === this.appointment.providerId)
      },

      servicesFiltered () {
        let availableServiceIds = []

        this.options.entities.employees.filter(
          employee => employee.status === 'visible'
        ).map(
          employee => employee.serviceList
        ).map(
          employeeServiceList => employeeServiceList.map(
            employeeService => employeeService.id
          )
        ).forEach(function (serviceIdList) {
          availableServiceIds = availableServiceIds.concat(serviceIdList)
        })

        return this.options.entities.services.filter(
          service =>
            (service.status === 'visible' && availableServiceIds.indexOf(service.id) !== -1) ||
            this.appointment.serviceId === service.id
        )
      },

      categoriesFiltered () {
        return this.options.entities.categories.filter(
          category => category.serviceList.filter(
            categoryService => (categoryService.status === 'visible' && (this.servicesFiltered.map(service => service.id).indexOf(categoryService.id) !== -1)) ||
              this.appointment.serviceId === categoryService.id)
            .length > 0
        )
      },
