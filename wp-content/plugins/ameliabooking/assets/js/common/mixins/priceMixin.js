export default {
  ...
  methods: {
    ...
    getCurrencySymbol () {
      return this.currencies.find(currency => currency.code === this.$root.settings.payments.currency).symbol
    },
    ...
    getFormattedPrice (price) {
      ...
      return pricePrefix + (j ? i.substr(0, j) + thousandSeparator : '') + i.substr(j).replace(/(\d{3})(?=\d)/g, '$1' + thousandSeparator) + (decimalPlaces ? decimalSeparator + Math.abs(price - i).toFixed(decimalPlaces).slice(2) : '') + priceSuffix
    }
  },
